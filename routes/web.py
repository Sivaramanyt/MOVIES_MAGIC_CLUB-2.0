...
async def search_movies(request: Request, q: str = ""):
    ...
    if db is not None and q.strip():
        # ✅ Search MOVIES (without seasons field)
        ...
        movies = [
            {
                ...
                "poster_path": doc.get("poster_path"),
                "type": "movie"  # ✅ Mark as movie
            }
            async for doc in movie_cursor
        ]

        # ✅ Search SERIES (with seasons field)
        series_cursor = db["movies"].find(
            {
                "title": {"$regex": q, "$options": "i"},
                "seasons": {"$exists": True}
            }
    ...
    context = {
        "request": request,
        "query": q,
        "movies": all_results,  # Combined results
    }

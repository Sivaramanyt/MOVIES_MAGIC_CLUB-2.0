...
async def search_movies(request: Request, q: str = ""):
    ...
    if db is not None and q.strip():
        # ✅ UNIFIED SEARCH - Find ALL matching documents (movies + series)
        ...
        all_results = [
            {
                ...
                "poster_path": doc.get("poster_path"),
                # ✅ Auto-detect if movie or series based on seasons field
                "type": "series" if doc.get("seasons") else "movie"
            }
            async for doc in cursor
        ]
    ...
    context = {
        "request": request,
        "query": q,
        "movies": all_results,  # Template still uses 'movies' variable
    }
